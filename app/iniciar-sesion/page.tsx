...
import { signInWithGooglePopup } from "@/app/auth-actions";
import { auth } from "@/auth";

export default async function SignInPage() {
  const session = await auth();
  if (session?.user?.email) redirect("/auth/completado");
  ...
  return (
    <main className="oauth-page">
      <section className="oauth-card" aria-labelledby="oauth-title">
        ...
        <h1 id="oauth-title">Entra a tu cuenta</h1>
        <p>Usa tu correo institucional de la UdeG.</p>
